...
import { estimateReadingTime } from '~/lib/articleUtils';
...
const ArticleReader: React.FC<ArticleReaderProps> = ({ item, onClose, onNavigate, onBookmark, hasPrev, hasNext, inline }) => {
  ...
  const readingTime = useMemo(() => estimateReadingTime(item.summary?.content ?? ''), [item.summary?.content]);
  const scrollRef = useRef<HTMLDivElement>(null);

  const sanitizedContent = useMemo(() => {
    const raw = item.summary?.content ?? '';
    const purified = DOMPurify.sanitize(raw, { ADD_ATTR: ['target'] });
    return cleanArticleContent(purified, item.title);
  }, [item.summary?.content, item.title]);
  ...
  const readerContent = (
    <div className={`flex flex-col ${inline ? 'h-full' : 'h-full sm:h-auto sm:gap-4'}`}>
      {/* ── Mobile top bar (modal only) ── */}
      {!inline && (
      ...
      {/* ── Navigation + meta bar ── */}
      <div className={`${inline ? 'flex' : 'hidden sm:flex'} items-center justify-between ${inline ? 'px-4 py-2 border-b border-airq-dark/10 flex-shrink-0' : ''}`}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-airq-dark/40 font-mono ml-2">{readingTime}</span>
        </div>
      ...
      {/* ── Scrollable content ── */}
      <div ref={scrollRef} className={`flex-1 overflow-y-auto min-h-0 ${inline ? 'px-4 py-4' : 'px-4 sm:px-0 py-4 sm:py-0'}`}>
        {/* Mobile meta line (modal only) */}
        {!inline && (
        ...
        {/* Feed name (inline only) */}
        {inline && (
          <p className="text-xs font-mono text-airq-dark/40 mb-1">{feedName}</p>
        )}

        {/* Title */}
        <h1 className="masthead text-2xl font-bold text-airq-dark leading-tight">
          {item.title}
        </h1>

        {/* Byline */}
        {item.author && (
          <p className="text-xs font-mono text-airq-dark/50 mt-1">
            {item.author}
          </p>
        )}
        ...
        {/* Article content */}
        <div
          className="prose prose-sm max-w-none text-airq-dark prose-headings:text-airq-dark prose-a:text-airq-contrast prose-a:no-underline hover:prose-a:underline prose-img:rounded mt-4"
          dangerouslySetInnerHTML={{ __html: sanitizedContent }}
        />
      </div>

      {/* ── Mobile bottom nav (modal only) ── */}
      {!inline && (
  ...
  if (inline) return readerContent;
